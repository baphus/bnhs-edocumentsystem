...
class RequestController extends Controller
{
        ...
        switch ($request->action) {
            case "delete":
                ...
                RequestLog::create([
                    ...
                    "user_id" => $request->user()->id,
                    "action" => "bulk_delete",
                    "description" => "Admin {$request->user()->name} bulk deleted {$count} requests",
                ]);
            ...
            case "resend_otp":
                ...
                foreach ($requests as $req) {
                    if ($req->status === "Pending" || !$req->otp_verified) {
                        $otp = $req->generateOtp();
                        // Send email with OTP (youll need to implement this)
                ...
                RequestLog::create([
                    ...
                    "description" => "Admin {$request->user()->name} bulk resent OTP for {$count} requests",
                ]);
    ...
    public function destroy(Request $request, DocumentRequest $documentRequest)
    {
        ...
        RequestLog::create([
            ...
            "description" => "Admin {$request->user()->name} deleted request {$trackingId}",
        ]);
